...
| kodi | Ubuntu 22.04's Kodi 19, Flathub's (21.x) | apt, flatpak, under Xvfb | the plugin:// search and download URLs Kodi's subtitle dialog opens, over JSON-RPC, for each file played by its path alone and then from Kodi's library, and the title and rung the addon logs; and that a download the API refuses ends the listing with the failure logged |
